...
export class BodychartComponent implements OnInit {
  ...
  showInfo(bodyPart: string) {
    ...
    switch (bodyPart) {
      case 'Chest':
        this.infoText = this.responseData.symptoms
          .find((symptom: any) => symptom.body_part === 'Chest')
          .symptoms.map((symptom: any) => symptom.symptom)
          .join(', ');
        break;
      case 'Arms, Jaw, Back':
        this.infoText = this.responseData.symptoms
          .find((symptom: any) => symptom.body_part === 'Arms, Jaw, Back')
          .symptoms.map((symptom: any) => symptom.symptom)
          .join(', ');
        break;
